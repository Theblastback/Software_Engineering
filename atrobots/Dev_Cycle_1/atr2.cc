...
void setscreen() {
	std::cout << "Begin setscreen" << endl << endl;
	int16_t i;
...
//	//outtextxy(3, 3, "FreeMem: " + cstr(memavail);  XXX Not possible. Cannot get amount of bytes available in heap storage (that's what memavail is)
	...
	for (i = 0; i <= MAX_ROBOTS; i++) {
		cout << "I: " << to_string(i) << "\tnum_robots: " << to_string(num_robots) << endl << endl;
		...
		if ( i < max_shown() ) {
			...
			if ( i <= num_robots ) {
				cout << "i is less than num_robots" << endl;
				...
				cout << "i is greater than num robots" << endl;
...
void create_robot(int16_t n, std::string filename) {
	std::cout << "create robot" << endl << endl;
