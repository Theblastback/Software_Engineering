...
string copy(string name, uint16_t start, uint16_t count) {
	...
	cout << "Ending name: " << temp << endl << "End copy" << endl;
	return temp;
}
...
string rstr(string s1, uint16_t l) {
	...
	if (s1.length() <= l) {
		...
		cout << "Rstr, length less than string" << endl;
